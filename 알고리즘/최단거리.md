...
다만 다익스트라 알고리즘은 그리디하게 동작해서 현재의 노드에서 가장 가까운 값으로 갈 수 있는 노드로 발전해나간다.

반면 벨만포드는 DP개념으로 작동해서 모든 경우의 수를 다 탐색해가면서 최소비용을 찾는다. 

기본적으로 벨만포드는 다익스트라 알고리즘보다 더 오래 걸리는 탐색방법이다

그러므로 모든 가중치가 양수인 경우 다익스트라 알고리즘을 쓰는것이 좋다

하지만 가중치에 음수가 있다면 벨만포드 알고리즘을 사용한다

또한 음의 사이클이 있는지 확인까지 할 수 있다.

알고리즘 구성

1) 모든 간선(edge)들을 탐색하면서 간선이 잇는 출발정점이 한번이라도 계산된 정점이라면 해당 간선이 잇는 정점의 거리를 비교해서 업데이트한다.(무엇을? 출발점에서 해당 정점까지의 최소 비용을!)
