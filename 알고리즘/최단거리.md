...
#### 2) 로직
1) 모든 간선(edge)들을 탐색하면서 간선이 잇는 출발정점이 한번이라도 계산된 정점이라면 해당 출발 정점에서 인접한 정점들까지의 최소비용을 비교해서 업데이트한다
2) 1번의 과정을 (정점의갯수 - 1) 만큼 반복한다

좀 더 구체적으로 설명해보자면
![[Pasted image 20231223182851.png|400x300]]  
최단거리를 구하기 위해 초기 세팅을 한다면  
출발점이 1번노드라면 1번노드의 누적거리값은 0이고, 나머지는 INF로 초기화하고 시작할 것이다.

즉 한번이라도 계산된 정점이란 노드의 누적거리값이 INF가 아닌 정점을 의미하며 해당 정점을 출발점으로 하는 간선을 파악해서 업데이트 하는 것 

알고리즘을 따라가보면

1) 간선 A는 1 -> 2로 가는 간선이며 1이 한번이라도 계산된 정점이고, 2의 비용이 INF이므로 정점2의 비용이 3으로 업데이트된다. 그리고 정점2는 이제 한번이라도 계산된 정점이 되었다.

2) 간선 B는 1 -> 3로 가는 간선이며 위와 같은 이유로 3번정점의 비용은 2로 업데이트되고, 3번정점도 계산된 정점이다.

3) 간선 C는 1 -> 4로 가는 간선이며 위와 같은 이유로 4번정점의 비용은 5로 업데이트되고, 4번정점도 계산된 정점이다.

4) 간선 D는 2 -> 3로 가는 간선이며 정점 2는 A간선을 계산할때 계산된 정점이 되어있는 상황이다.

    또한 정점3은 간선B를 계산할때 비용이 2로 업데이트되어있다.

    즉 간선 D의 상황을 보면 1 -> 2에서 3의 비용, 2 -> 3에서 3의 비용으로 1 -> 3의 비용은 6이 되고

    기존의 값인 2보다 크므로 업데이트하지 않는다.

5) 간선 E는 3 -> 4로 가는 간선이며 3은 계산된 정점이고, 현재 4의 비용은 5이다

    즉 1 -> 3의 비용 2, 3 -> 4의 비용 -4로 1 -> 4의 비용은 -2가 된다.

    기존에 4의 비용이었던 5보다 -2가 작으므로 정점 4의 비용은 -2로 업데이트된다.
