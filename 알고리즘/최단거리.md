...
| O(E)                                                         | 시작점에서 각 점까지의 최단거리를 구할 경우 O(ElogV)   | O(VE)                                                                            | O(V^3)                                                                                                 |
...
| INF 미사용                                                   | INF 사용                                                       | INF 사용                                                                                 | INF / 2 사용                                                                                                       |
...
위 알고리즘을 사용해서 위의 그래프를 탐색한다면 시작점 1을 방문하고나서 우선순위큐에 (2, 2)와 (12, 3)가 들어가게 됨(우선순위 큐에서 정렬을 편하게 하기 위해 누적거리를 앞에 둠)  
그 후 탐색을 계속 하다보면 (9, 3)도 우선순위큐에 들어가게 되며 `distance[3]`는 9로 갱신되고 이미 큐에 들어가있는 (12, 3)가 꺼내졌을때는 `distance[3]`와 비교해서 더 크므로 무시하게 됨
...
std::vector<int> dijkstra(int start) {
	...
	while (!pq.empty()) {
		...
		for (auto& p : adj[here]) {
			...
			int thereCost = hereCost + p.first;	// 현재 방문한 정점을 거쳐서 다음 정점을 갈때의 비용 
