...
다익스트라 알고리즘에서는 오름차순 정렬을 원하므로  
`std::priority_queue< std::pair<int, int>, std::vector<std::pair<int, int>>, std::greater<> > pq;`형태로 우선순위큐를 선언하거나  
그냥 `std::priority_queue<std::pair<int, int>> pq;`로 선언하고 
`19line // int cost = -pq.top().first;`
`28line // pq.push(std::make_pair(-thereCost, there));`로 두 줄을 -로 넣고 빼오게 작성하는 방법도 있음