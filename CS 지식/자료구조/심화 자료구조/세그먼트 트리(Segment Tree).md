...
1) 현재 탐색하는 범위에 찾고자 하는 범위가 포함되는 경우(또는 완벽히 일치하는 경우)
...
또는 index 0 ~ 3의 구간합을 구한다면 2번노드의 값은 0 ~ 2이므로 완전히 속하므로 이 값을 리턴한 후 리턴된 값들끼리 더해서 결과를 낼 수 있으므로 2번노드의 값인 6을 return한다.
...
세그먼트트리는 값뿐만 아니라 구간합들도 저장되어있으므로  
리프노드인 배열의 값이 변경되면 연관된 구간합 값들도 모두 변경해야한다.

트리의 값을 변경할 때는 2가지 경우의 수가 있다.
1) 바꾸고자 하는 index값이 현재 탐색하는 범위에 속해있지 않은 경우
2) 바꾸고자 하는 index값이 현재 탐색하는 범위에 속해있는 경우

1번의 경우 값을 변경하지 않아도 되므로 탐색을 더 하지 않으면 되고 2번의 경우가 문제인데  
![[Pasted image 20240323133243.png|300x300]]
`arr[] = {1, 2, 3, 4, 5};`일때 배열의 index 1의 값을 5로 바꾸라고 한다면  
1번 노드에서 시작해서 왼쪽서브트리와 오른쪽서브트리로 진행해나가게 된다.  
