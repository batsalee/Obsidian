...
왜냐하면 (1, 0, 4)에서 노드번호는 1 * 2로 2가 된것이고 왼쪽범위는 0~2이므로 

여기서 또 왼쪽재귀이므로 다음 진행은 (4, 0, 1)이 될것이다.

그 다음은 (8, 0, 0)이 된다. 이때 시작하는범위==끝나는범위일때 재귀를 종료한다.
...
(9, 1, 1)이 된다. 이 또한 시작하는범위==끝나는범위이므로 

SegmentTree[9] = arr[1]; 이 된다.
