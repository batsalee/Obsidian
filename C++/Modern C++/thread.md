...
 다 합치면  실행시간이 거의 10분의 1 수준이 될 수 있다.  
...
즉 t1이 끝나고 리턴을 해야 t1.join() 코드를 수행한 셈 치고 다음 줄로 넘어가는 것이다.
...
쓰레드는 리턴값이 없기때문에 결과를 받고싶다면 쓰레드가 실행시킬 함수의 매개변수에 포인터 변수를 놓고 거기에 전달 받으면 된다.
...
예를들어 t1이 count 값을 확인했을때 10이어서 11로 만든 후 count = 11을 대입한다.
...
위처럼 lock_guard<mutex> lock(m)으로 lock 객체를 만들고, 스코프를 벗어날때 unlock하게 할 수 있다.

또는 unique_lock<mutex> lock(m)으로도 사용 가능하다.
