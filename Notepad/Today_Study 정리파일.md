...
동적할당은 애초에 스마트포인터 쓰는게 낫다.
다만 꼭 사용해야 하는 경우
동적할당시 반드시 해제해줘야한다. 그리고 해제 후에는 포인터값을 nullptr으로 초기화해준다.  
```C++ 
int* p = new int;
*p = 10;
...
또한 동적할당한 변수를 delete하지 않고 프로그램이 종료되는 경우 동적할당된 변수가 자동으로 회수된다고 볼 수는 없다.  
운영체제에 의해 해당 변수가 회수되는것이고, 프로그램에 의해 회수되는게 아니다.  
그래서 회수 시점을 정확히 예측할 수 없고 그 기간동안은 메모리릭이 발생하므로 프로그램이 종료된다고 해도 동적할당은 delete하는게 좋다.  
...
포인터 변수는 항상 nullptr으로 초기화한다. 포인터 변수를 사용하기 전에는 nullptr이 아닌지 비교한다.  
```C++
int *p = nullptr;
if(p != nullptr) ~~~
```
...
스마트포인터를 쓰면 그만일것 같긴 하다.

3) 소멸자에서는 예외를 절대로 밖으로 보내면 안된다.  
소멸자를 호출한 함수가 따로 있는게 아니다보니 소멸자에서 예외를 밖으로 보내면 프로그램이 강제종료되므로 소멸자 내부의 내용을 전부 다 try로 감싸주고 catch(...) {} 로 소멸자 안에서 처리해줘야한다  
...
move함수는 좌측값을 우측값 레퍼런스로 캐스팅해주는 함수
```C++
template <typename T>
...
```
위같은 상황에서 a, b가 아주 크면 안좋으니까 이럴때 좌측값을 우측값 레퍼런스로 만들고, operator=도 구현해서  
```C++
template <typename T>
void my_swap(T &a, T &b) {
...
```
`foward<T>(a)`는 a가 좌측값일땐 좌측값 레퍼런스로, 우측값일땐 우측값 레퍼런스로 캐스팅한다.



C++에서 메모리를 잘못된 방식으로 관리하면 두가지 문제점이 생길 수 있음
1) 메모리 릭(이건 RAII패턴을 사용해서 해결 가능)
2) double free(이미 해제된 메모리를 다시 참조하는 경우)
...
해시테이블 : 데이터를 담을 테이블을 "미리 크게 만들어두고" 입력받은 데이터를 해싱해서 key-value로 저장하는 구조
...
ref()는 복사 가능한 레퍼런스, 레퍼런스 객체라 생각하면 컨테이너에 보관도 가능하다.
사실상 레퍼런스 래퍼를 반환하는 형변환 함수
쓰레드로 실행하는 함수는 레퍼런스를 매개변수로 받아도 그냥 값 넘기면 복사해버림, ref로 넘겨야 함
...
\#최단거리
다익스트라 : 그리디, priority_queue
벨만포드 : 음의 사이클 유무도 확인 가능, DP, 다익스트라보다 오래걸림
	간선 탐색하면서 선의 출발지가 INF가 아니면 보는것(N-1회)
	간선 객체를 만들어서 사용
플로이드와샬 : 모든 정점에서 모든 정점까지의 거리 확인 가능, 단 음의 사이클 있으면 사용불가

C++ algorithm의 sort()는 그냥 퀵소트를 쓰는게 아니다.
IntroSort를 사용한다라고 말하는게 맞으며, 기본적으로는 퀵소트를 쓰는데 퀵정렬의 피봇설정이 안좋아서 재귀 깊이가 한계에 도달하면 힙정렬로 전환한다.
그리고 아주 작은 크기의 데이터는 삽입정렬을 사용하는게 퀵정렬보다 빠르므로 작은 데이터는 삽입정렬을 쓴다.

이분탐색은 정렬하고 써야한다, 당연히 lower_bound, upper_bound도 정렬하고 써야한다

유니온파인드 == 분리집합 == 서로소집합 : 꼭 그래프에서만 쓰는게 아니라 서로 공통원소가 없는 부분집합에서도 사용할 수 있따.
유니온파인드는 자기 자신의 부모를 자기자신으로 설정하고 시작(1개씩 동떨어진 트리모양새로 시작), 모든 노드를 합치기 위한 알고리즘이고 합치면서 더 작은값을 부모로 설정

MST(최소 신장 트리) : 크루스칼 프림
신장트리 : 모든 노드가 연결되어 있되 간선의 갯수가 최소(그럼 당연히 간선은 N - 1개고 사이클이 없음)
최소신장트리 : 모든 노드가 연결되고 있되 간선의 가중치 합이 가장 작은것
최소신장트리를 만드는 알고리즘이 크루스칼 프림이다.
크루스칼 : 간선을 선택하며 진행, 프림 : 노드를 선택하며 진행

\#크루스칼
간선의 가중치를 오름차순 정렬, 가장 가중치 낮은 간선부터 두 정점 같은 그룹 아니면 연결 반복

\#소켓
서버
1) 소켓생성 : 이때 TCP인지 UDP인지
2) bind : ip, port 지정
3) listen
4) 요청 오면 accept
클라
...
알고리즘헤더
erase가 지우는거, remove가 밀어주는거

문자열내에서 문자열 탐색할때
알고리즘 헤더의 search()를 떠올려야 한다.
특히나 보이어무어 알고리즘도 적용 가능하다

github 
.zip은 파일만 다운받는 것
clone은 git까지 다운받는 것
fork는 본인 레포지토리로 가져오는 것, git은 못받음

자료구조 heap은 완전이진트리 형태로 구현한다.
완전이진트리란 위에서부터, 왼쪽에서부터 채워나가는 방식

vector의 단점
1) 공간이 가득 차면 새로 할당하고 기존 값들 다 옮겨줘야 헤서 O(N)이 될 수 있음
2) bool의 템플릿 특수화
3) shrink_to_fit 문제
4) insert나 erase하면 iterator가 무효화됨
5) 마찬가지로 확장을 위해 새로운 공간으로 값을 옮길때도 itr이 무효화됨

deque의 단점
1) 앞뒤 공간을 다 비워야하고, 블록들의 위치를 기억하는 테이블도 필요하므로 메모리를 더 많이 사용함
근데 메모리 단편화를 막아줄수도 있어서 꼭 그렇다고 보기도 애매한듯
2) 데이터가 들어있는 블록들이 메모리에 꼭 연속적으로 존재하지 않을 수 있음
그러므로 포인터 연산 사용이 애매할 수 있음

iterator end()는 끝보다 한칸 뒤를 가리킨다.

unordered_set과 unordered_map은 기본자료형이 아닌, 내가 만든 클래스같은걸 저 컨테이너에 넣을때
해시함수도 내가 작성해줘야 하기때문에 속도가 아주 중요한 경우가 아니면 사용하지 않는다.
또한 unordered들은 해시테이블의 단점을 그대로 가져간다.
1) 저장한 데이터가 너무 적다면 오히려 해시함수를 수행하는 오버헤드가 손해가 된다.
2) 반대로 저장한 데이터가 너무 많다면 해시 충돌이 발생해서 O(1)이 아니라 O(N)이 될수도 있다.
3) 그렇다고 새로 더 큰 공간을 할당하고 옮기려면 rehash해줘야 하니 또 손해가 된다.

소프트웨어공학
SRS : 객체지향분석(OOA)과정에 작성, 소프트웨어 요구 명세서, 개발자/PM/고객이 만나서 뭘 만들지 작성하는 명세서, 유스케이스 다이어그램 활용
SDS : 객체지향설계(OOD)과정에 작성, 소프트웨어 디자인 명세서 : 구체적으로 어떻게 만들지 작성, 시스템 구조나 DB구조나 알고리즘 등, 클래스다이어그램, 시퀀스 다이어그램 활용

헐리우드 원칙
의존성 부패를 막기 위한 원칙
상위모듈이 하위모듈에 의존하고, 하위모듈이 상위모듈에 의존하면 의존성 부패가 생긴다
그러니 상위모듈이 하위모듈을 호출할땐 추상화된 인터페이스를 호출해야 한다.
즉 구체화에 의존하지 말고 추상화에 의존해야 한다.(템플릿 메서드 패턴처럼 구현)

디자인패턴을 좀 더 큰 분류로 따진다면
아키텍쳐패턴, 디자인패턴, 이디엄 패턴으로 나눌 수도 있다.
예시로는 아키텍쳐패턴이 MVC, MVP, MVVM같은것
디자인 패턴이 생성/구조/행동패턴
이디엄 패턴이 RAII패턴 같은 것

빌더패턴의 핵심은 별도의 빌더 클래스를 만들고, 마지막에 .build()하는 것
장점은 필수멤버변수와 선택멤버변수를 나눠서 구성하기 좋고
미리 설정만 해두고 마지막에 한번에 다 build()해서 동시에 생성해버릴 수 있다.
단점은 코드 복잡성 증가 외에도 생성자보다는 성능이 떨어진다는 점

브릿지의 대표적 예시 pimpl
C++에서 구현 세부사항은 숨기고 추상 인터페이스만 노출시키는 브릿지 패턴의 예시

컴파짓 : 단순하게 설명하면 "사용자가 단일객체와 복합객체를 동일한 방법으로 다룰 수 있게 만드는 패턴"
단일객체와 복합객체는 공통의 조상클래스를 가짐
두 객체를 다루는 방법이 동일해야하며, 동일한 그 함수는 조상클래스에 위치함

프록시
게으른 초기화
프록시가 원본 클래스와 동일한 인터페이스를 제공해야한다.
만약 접근했을때 원본객체가 없다면 게으른초기화로 생성한다. 있다면 원본 객체의 해당함수를 호출한다.
사용 예시로는 ProxyA에서 A에 대한 접근 권한을 검증하거나, 접근 방식을 최적화하는 경우 등이 있다.

데코레이터의 최대 장점은 동일 클래스내의 특정 객체 하나만 꾸며주는게 가능하다는것
만약 클래스를 다 분화해서 만들면 클래스자체가 그냥 틀이 되는건데 데코레이터로 구현하면 
객체 한개만 특별하게 꾸밀 수 있음
데코레이터의 설명은 커피에 초점이 아니라 재료에 초점이 있다.
재료가 추가된다면? 조합수가 어마어마하게 늘어난다. 이걸 커버한다.
빌더패턴과 굉장히 유사함

빌더 vs 데코레이터
빌더는 생성시점에 조합을 잘 적용해서 만든다.
문제점은 새로운 옵션이 생기면 기존 클래스를 수정해야하니 OCP를 위반한다.
데코레이터는 생성후에 구조를 변경하는 개념이다.
문제점은 새로운 옵션이 생길때마다 클래스를 신규로 계속 찍어내야한다.

커맨드 : 요청을 객체화해서 모아서 처리하거나 되돌릴 수 있다.
스테이트 : 상태를 객체화한다.
옵저버 : 객체가 본인의 상태가 변하면 옵저버들에게 알리는 것

브릿지랑 커맨드는 나중에 다시 한번 조사해서 공부하는게 좋을듯

메멘토는 하나의 커밋, 케어테이커가 깃

복사 생성자는 생성시점에 복사를 하건, 대입을 하건 호출된다.
대입연산자는 생성자가 아닌때에 사용되는것이다.
디폴트 복사생성자는 얕은 복사를 수행하므로 깊은 복사를 수행하고 싶다면 구현해줘야한다.

부동소수점은 값은 더 크게 보관가능한데 정확도가 떨어진다.
고정소수점은 값을 더 적게 보관할 수 있지만 정확도는 더 높아진다.

list가 중간에 값을 넣고빼는게 항상 유리하다고 할 수는 없다.
값의 탐색이 O(N)이고 값의 삽입삭제가 O(1)이라 결국 O(N)이 될 수 있다.
반면 set같은건 탐색도 삽입삭제도 O(logN)형태니까 오히려 유리할 수 있다.

크루스칼 : 간선을 오름차순 정렬, 유니온파인드로 검사하면서 연결 N-1개까지